...
export default async function handler(req, res) {
  ...
  if (!apiKey) {
    return res.status(500).json({ error: "Missing ODDS_API_KEY in backend env" });
  }

  const sport = "americanfootball_nfl";
  const url = `https://api.the-odds-api.com/v4/sports/${sport}/odds/?apiKey=${apiKey}&regions=us&markets=h2h,spreads&oddsFormat=american`;
