...
function renderGame(ev, odds) {
  ...
  // Calculate favorite by average implied probability
  const avgOdds = team => {
    const prices = [];
...
async function loadProps(id, container) {
  ...
  for (const k of order) {
    ...
    html += `
      ...
      <table class="table">
        <thead>
          <tr><th>Book</th><th>Player</th><th>Pick</th><th>Line</th><th>Price</th></tr>
        </thead>
        <tbody>
          ${arr
            .slice(0, 40)
            .map(
              r =>
                `<tr><td>${r.book}</td><td>${r.player}</td><td>${r.name}</td><td>${r.point ?? "–"}</td><td>${money(
                  r.price
                )}</td></tr>`
            )
            .join("")}
